...
class Render:
  ...
  def render(self, path, clean=True, intensity=3.0, mesh=None, only_render_images=False):
    ...
    rendered_image, _ = pyrender_rendering(
        mesh1, viz=False, light=True, camera_pose=self.camera_pose, intensity=intensity, bg_color = self.background
    )
  ...
  def get_program(
    ...
    if self.program is None:
      ...
      self.program = pyrender.shader_program.ShaderProgram(
          current_work_dir + "/shades/mesh.vert", current_work_dir + "/shades/mesh.frag", defines=defines
      )
    return self.program
...
def pyrender_rendering(mesh, camera_pose, viz=False, light=False, intensity=3.0, bg_color=None):
  # renderer
